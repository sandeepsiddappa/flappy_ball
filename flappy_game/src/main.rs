...
Dependencies : ggez for front-end(graphics) and random 
...
impl GameReq {
    ...
    fn add_or_remove_pipes(&mut self) {
        ...
        let need_new_pipe = self.pipes.is_empty() || self.pipes.last().unwrap().x_axis < SCREEN_WIDTH - 300.0;
        ...
        match self.state {
            GameReqType::Start => {
                ...
                graphics::draw(
                    ctx,
                    &start_text,
                    ([SCREEN_WIDTH / 2.0 - 100.0, SCREEN_HEIGHT / 2.0],),
                )?;
            }
            GameReqType::Paused => {
...
mod tests {
    ...
    fn is_ball_out_of_bounds_test() {
        ...
        assert_eq!(
            game.state,
            GameReqType::GameOver,
            "Game should end when ball goes out of bounds."
        );
    }
    ...
    fn collision_with_pipe_test() {
        ...
        assert_eq!(
            game.state,
            GameReqType::GameOver,
            "Game should end when ball collides with pipe at position ({}, {})",
            game.ball.x_axis,
            game.ball.y_axis
        );
    }
    ...
    fn no_score_increment_without_pipe_pass_test() {
        ...
        assert_eq!(
            game.score, 0,
            "Score should not increment when the ball hasn't passed a pipe."
        );
    }
    ...
    fn ball_movement_test() {
        ...
        assert!(
            game.ball.y_axis < initial_y,
            "Ball should move upward when jumping"
        );
    }
    ...
    fn pipe_generation_test() {
        ...
        assert_eq!(
            pipe.x_axis, SCREEN_WIDTH,
            "New pipe should start at screen width"
        );
    }
